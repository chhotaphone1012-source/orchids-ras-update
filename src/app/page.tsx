...
import { Gamepad2, Users, Trophy, Star, Target, Crown, Sparkles, Flame, Rocket, Play, ArrowRight, Clock } from "lucide-react";
...
const games = [
  ...
  { id: "educational-adventures", name: "Educational Adventures", icon: "🧠", desc: "Learn Math, Science, History & more while playing!", color: "from-purple-500 to-blue-600", disabled: true },
  { id: "endless-runner", name: "Endless Runner", icon: "🏃", desc: "Run, jump, slide! Collect coins and avoid obstacles!", color: "from-indigo-500 to-purple-600", disabled: true },
];
    ...
    useEffect(() => {
      ...
      try {
        const unsub = onSnapshot(collection(db, "banners"), (snap) => {
          if (!snap.empty) {
            const docs = snap.docs.map(d => d.data() as any).filter(b => b.image && b.title);
            if (docs.length > 0) setBanners(docs);
          }
        });
        return () => unsub();
      } catch (e) {
    ...
    <div ref={containerRef} className="flex flex-col w-full dark galaxy-bg min-h-screen overflow-x-hidden">
        ...
        >
          <div className="flex items-center justify-center gap-3 sm:gap-6 mb-10">
              <motion.div
                ...
                className="w-12 h-12 sm:w-16 sm:h-16 md:w-24 md:h-24 rounded-2xl md:rounded-3xl gold-gradient flex items-center justify-center animate-pulse-glow shadow-2xl shrink-0"
              >
                <Gamepad2 className="h-7 w-7 sm:h-10 sm:w-10 md:h-14 md:w-14 text-black" />
              </motion.div>
              <div className="min-w-0">
                <h1 className="text-5xl sm:text-7xl md:text-[13rem] font-black gold-text-gradient tracking-tight leading-none">
                  6GAMER
                </h1>
                <div className="h-2 md:h-3 gold-gradient rounded-full mt-2 md:mt-4 shadow-[0_0_20px_rgba(251,191,36,0.5)]" />
              </div>
              <motion.div
              ...
              >
                <Sparkles className="w-10 h-10 md:w-16 md:h-16 text-yellow-500" />
              </motion.div>
            </div>
      ...
      <motion.section 
        ...
        transition={{ duration: 1 }}
          className="relative h-[50vh] sm:h-[65vh] overflow-hidden mx-3 sm:mx-6 rounded-2xl sm:rounded-[3rem] z-10 gold-border-glow shadow-2xl"
      >
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-black/20 flex flex-col items-center justify-center text-center p-4 sm:p-8">
                  <h2 className="text-3xl sm:text-5xl md:text-7xl font-black gold-text-gradient mb-3 sm:mb-6 drop-shadow-2xl uppercase">
                    {banners[currentBanner].title}
                  </h2>
                  {banners[currentBanner].category && (
                    <Badge className="mb-3 sm:mb-6 gold-gradient text-black font-black uppercase tracking-[0.2em] px-4 sm:px-6 py-1 sm:py-2 rounded-full text-xs sm:text-sm">
                      {banners[currentBanner].category}
                    </Badge>
                  )}
                  <p className="text-base sm:text-2xl md:text-3xl text-orange-200 max-w-4xl font-bold mb-6 sm:mb-10 drop-shadow-lg uppercase italic">
                    {banners[currentBanner].subtitle}
                  </p>
                  <Link href={banners[currentBanner].link || "/signup"}>
                    <Button size="lg" className="gold-gradient text-black text-base sm:text-2xl px-6 sm:px-12 py-4 sm:py-8 rounded-full animate-pulse-glow hover:scale-110 transition-transform font-black shadow-[0_0_30px_rgba(251,191,36,0.6)]">
                      <Play className="w-5 h-5 sm:w-8 sm:h-8 mr-2 sm:mr-3 fill-current" /> {banners[currentBanner].link ? "EXPLORE NOW" : "PLAY NOW"}
                    </Button>
                  </Link>
                </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            >
              ...
              <h2 className="text-4xl sm:text-6xl md:text-7xl font-black gold-text-gradient mb-6 uppercase">Our Best Games</h2>
                      ...
                      return (
                        <motion.div
                          ...
                          animate={{
                            ...
                            x: isCenter ? 0 : isLeft ? (typeof window !== "undefined" && window.innerWidth < 640 ? -200 : -450) : (typeof window !== "undefined" && window.innerWidth < 640 ? 200 : 450),
                            z: isCenter ? 0 : -500,
                          ...
                          className="absolute"
                        >
                          <div className={`w-[280px] sm:w-[380px] p-6 sm:p-12 rounded-2xl sm:rounded-[3rem] gold-border-glow bg-black/95 backdrop-blur-2xl text-center ${isCenter ? "shadow-[0_0_80px_rgba(251,191,36,0.4)]" : ""}`}>
                            <motion.div 
                              className={`w-[100px] h-[100px] sm:w-[160px] sm:h-[160px] rounded-2xl sm:rounded-[2.5rem] bg-gradient-to-br ${game.color} mx-auto mb-6 sm:mb-10 flex items-center justify-center shadow-2xl relative overflow-hidden`}
                              animate={isCenter ? { y: [0, -20, 0], rotate: [0, 10, -10, 0] } : {}}
                              transition={{ duration: 3, repeat: Infinity }}
                            >
                              <div className="absolute inset-0 shimmer-effect opacity-30" />
                              <span className="text-5xl sm:text-7xl relative z-10">{game.icon}</span>
                            </motion.div>
                            <h4 className="text-2xl sm:text-4xl font-black gold-text-gradient mb-3 sm:mb-6 uppercase">{game.name}</h4>
                            <p className="text-sm sm:text-lg text-orange-200 mb-6 sm:mb-10 font-bold leading-relaxed italic hidden sm:block">"{game.desc}"</p>
                            {isCenter && (
                              <Button 
                                  onClick={() => (isAdmin || !game.disabled) && handlePlayNow(game.id)}
                                  disabled={!isAdmin && game.disabled}
                                  className={`gold-gradient text-black font-black px-6 sm:px-10 py-4 sm:py-7 text-base sm:text-xl rounded-full hover:scale-110 transition-transform shadow-2xl ${(!isAdmin && game.disabled) ? 'opacity-50 cursor-not-allowed' : 'animate-pulse-glow'}`}
                                >
                                  {isAdmin && game.disabled ? "TEST MODE" : game.disabled ? "SOON" : "PLAY NOW"} <ArrowRight className="ml-2 w-5 h-5 sm:w-6 sm:h-6" />
                                </Button>
                            )}
                          </div>
                        </motion.div>
          ...
          <div className="text-center mt-20">
            <Link href="/games">
              <Button className="gold-gradient text-black px-10 sm:px-16 py-6 sm:py-9 rounded-full text-xl sm:text-2xl hover:scale-110 transition-transform font-black shadow-[0_0_40px_rgba(251,191,36,0.5)]">
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
                <h2 className="text-4xl sm:text-6xl md:text-7xl font-black gold-text-gradient mb-10 leading-tight uppercase tracking-tighter">Best Way to Play</h2>
            ...
            >
              <div className="aspect-square rounded-[3rem] overflow-hidden gold-border-glow shadow-2xl relative">
                ...
                <motion.div 
                  ...
                  transition={{ duration: 4, repeat: Infinity }}
                  className="absolute -top-5 -right-5 sm:-top-10 sm:-right-10 w-16 h-16 sm:w-28 sm:h-28 gold-gradient rounded-xl sm:rounded-[2rem] flex items-center justify-center shadow-2xl animate-pulse-glow"
                >
                  <Trophy className="h-8 w-8 sm:h-14 sm:w-14 text-black" />
                </motion.div>
                <motion.div 
                  animate={{ y: [0, 20, 0], x: [0, -12, 0] }}
                  transition={{ duration: 3.5, repeat: Infinity, delay: 0.5 }}
                  className="absolute -bottom-5 -left-5 sm:-bottom-10 sm:-left-10 w-14 h-14 sm:w-24 sm:h-24 bg-gradient-to-br from-orange-500 to-red-600 rounded-xl sm:rounded-[1.5rem] flex items-center justify-center shadow-2xl p-2 sm:p-4"
                >
                  <Flame className="h-8 w-8 sm:h-14 sm:w-14 text-white" />
                </motion.div>
            </motion.div>
          </div>
        </div>
      ...
      >
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            <h2 className="text-4xl sm:text-5xl md:text-6xl font-black gold-text-gradient mb-6 tracking-tight uppercase">Ready to Play?</h2>
            <p className="text-lg sm:text-2xl mb-12 text-orange-200 max-w-3xl mx-auto font-bold leading-relaxed uppercase italic">Start playing now and be the best on our list!</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
              <Link href="/signup">
                <Button size="lg" className="gold-gradient text-black px-8 sm:px-14 py-6 sm:py-8 text-xl sm:text-2xl rounded-full hover:scale-110 transition-all font-black shadow-2xl">
                  <Sparkles className="w-6 h-6 sm:w-7 sm:h-7 mr-2 sm:mr-3" /> SIGN UP
                </Button>
              </Link>
              <Link href="/leadership">
                <Button size="lg" variant="outline" className="px-8 sm:px-14 py-6 sm:py-8 text-xl sm:text-2xl rounded-full border-4 border-yellow-500 text-yellow-500 hover:bg-yellow-500/10 transition-all font-black hover:scale-110">
                  <Trophy className="w-6 h-6 sm:w-7 sm:h-7 mr-2 sm:mr-3" /> BEST PLAYERS
              </Button>
            </Link>
          </div>
        </div>
